  ...
  runApp(new MaterialApp(
	  ...
	  onGenerateRoute: (RouteSettings settings) {
		  ...
		  /*case '/g': return defaultTargetPlatform == TargetPlatform.iOS
              ? new CupertinoPageRoute(builder:  (_) => new Map(),settings: settings,)
              :new MyCustomRoute1(
            builder: (_) => new Map(),
            settings: settings,
          );*/
...
class MyCustomRoute1<T> extends MaterialPageRoute<T> {
  ...
  @override
  Widget TransitionBuilder(BuildContext context, Animation<Offset> animation, Animation<double> secondaryAnimation, Widget child) {
    if (settings.isInitialRoute)
      return child;
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: MyHomePage(title: 'SuperTOll'),
    );
  }
}*/
